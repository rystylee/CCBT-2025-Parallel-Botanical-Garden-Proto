...
### 4.0 タイムスタンプ管理の修正（優先度: 最高）

**現在の問題点**

1. **フィルタリングのタイミング**
   - 現状: `_receiving_phase()`終了時に`_filter_old_data()`を呼び出し
   - 問題: データ受信からフィルタリングまでにラグがある
   - 修正: `add_input()`メソッド内で即座にタイムスタンプをチェック

2. **タイムスタンプの誤った更新**
   - 現状: `_output_phase()`で`timestamp = time.time()`として新規タイムスタンプを発行
   - 問題: 人間からの元のタイムスタンプが失われる
   - 修正: バッファ内の最新のタイムスタンプを取得して使用
...
def add_input(self, timestamp: float, text: str, source_type: str, lang: str):
    ...
    logger.info(f"Added input: {source_type} '{text[:20]}...' (buffer size: {len(self.input_buffer)})")

# 2. _output_phase() メソッド
async def _output_phase(self):
    ...
    try:
        self.osc_client.send_to_all_targets(
            targets, "/bi/input", newest_timestamp, self.generated_text, "BI", lang
        )
...
async def _receiving_phase(self):
    ...
    # No longer need _filter_old_data() here
    logger.info(f"Buffer size: {len(self.input_buffer)}")
    self.state = "GENERATING"

# 4. _filter_old_data() メソッドは削除または非推奨化
```

**タイムスタンプ選択の方針**
- 複数入力時は最新のタイムスタンプを使用
- 理由: データの鮮度を正確に反映し、古いデータとして誤って破棄されるリスクを低減
